	...
	if (!$stmt->execute()) {
		...
		if ($result->num_rows > 0) {
			...
			if ($company_by_name['response']['code']=== "400" && $all_companies_location['response']['code'] === "400") {
					$companies['response']['code'] = "400";
					$companies["response"]["commments"]="Error no company or city was found.";
					return json_encode($companies);
			}
...
function job($job){
	...
	if (!($stmt = $mysqli->prepare("SELECT job.job_title,job.job_salary,company.company_name, city.city_name FROM
		job right join city on job.city_id = city.city_id right join company on job.company_id = company.company_id
WHERE job.job_title = ?"))) {
			echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
		}
	...
	if (!$stmt->execute()) {
	  ...
	  $result = $stmt->get_result();
		...
		if ($result->num_rows > 0) {
			...
			{
				$companies["response"]["code"]="400";
				$companies["response"]["commments"]="Error job_city";
				return json_encode($companies);
			}
			elseif ($job_by_name["response"]["code"] === "400") {
